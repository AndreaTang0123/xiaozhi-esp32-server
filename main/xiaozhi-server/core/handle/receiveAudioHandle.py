...
async def startToChat(conn, text, audio_data=None):
    ...
    conn.executor.submit(conn.chat, actual_text, audio_data=audio_data)
...
async def no_voice_close_connect(conn: "ConnectionHandler", have_voice):
    ...
    # Prevent idle timeout while LLM is actively thinking/speaking
    if hasattr(conn, "llm_finish_task") and not conn.llm_finish_task:
        conn.last_activity_time = time.time() * 1000
        return
    ...
    if conn.last_activity_time > 0.0:
        ...
        if (
            ...
            if end_prompt and end_prompt.get("enable", True) is False:
                conn.logger.bind(tag=TAG).info("Closing connection due to inactivity.")
                await conn.close()
                return
            prompt = end_prompt.get("prompt")
            if not prompt:
                prompt = "Please provide a very short, polite goodbye in the user's language to end the conversation since it has been idle."
