...
class CaptureOutput:
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        ...
        # Log captured content through logger
        if self.output:
        ...
        try:
            for file_name, file_path in model_files.items():
                if not os.path.isfile(file_path):
                    ...
                    if not os.path.isfile(file_path):
                        raise FileNotFoundError(f"Failed to download model file: {file_path}")
    ...
    async def speech_to_text(
        ...
        try:
            ...
            logger.bind(tag=TAG).debug(
                f"Audio file saved in: {time.time() - start_time:.3f}s | Path: {file_path}"
            )
            ...
            if self.delete_audio_file and file_path and os.path.exists(file_path):
                try:
                    os.remove(file_path)
                    logger.bind(tag=TAG).debug(f"Temporary audio file deleted: {file_path}")
                except Exception as e:
