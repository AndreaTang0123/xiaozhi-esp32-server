...
class ToolType(Enum):
    NONE = (1, "Do nothing after calling tool")
    WAIT = (2, "Call tool and wait for function return")
    CHANGE_SYS_PROMPT = (3, "Change system prompt, switch role character or responsibility")
    SYSTEM_CTL = (
        4,
        "System control, affecting normal dialogue flow like exit, play music etc, requires conn parameter",
    )
...
class ActionResponse:
    def __init__(self, action: Action, result=None, response=None):
        ...
        self.response = response  # Content of direct response
...
def register_function(name, desc, type=None):
    """Decorator to register function to function registry dictionary"""
...
def register_device_function(name, desc, type=None):
    """Decorator to register device-level function to function registry dictionary"""
