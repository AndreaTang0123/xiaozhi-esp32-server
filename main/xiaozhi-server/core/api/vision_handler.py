    ...
    async def handle_post(self, request):
        ...
        try:
            ...
            if not is_valid_image_file(image_data):
                raise ValueError(
                    "Unsupported file format, please upload a valid image file (supports JPEG, PNG, GIF, BMP, TIFF, WEBP)"
                )

            # Convert image to base64 encoding
            image_base64 = base64.b64encode(image_data).decode("utf-8")

            # If control panel is enabled, get model config from it
            ...
            if not select_vllm_module:
                raise ValueError("You have not set the default vision analysis module")
            ...
            if not vllm_type:
                raise ValueError(f"Cannot find provider {vllm_type} for VLLM module")
            ...
            self.logger.bind(tag=TAG).error(f"MCP Vision POST request exception: {e}")
            return_json = self._create_error_response("Error occurred while processing request")
            response = web.Response(
                text=json.dumps(return_json, separators=(",", ":")),
                content_type="application/json",
            )
    ...
    async def handle_get(self, request):
        ...
        try:
            ...
            if vision_explain and len(vision_explain) > 0 and "null" != vision_explain:
                message = (
                    f"MCP Vision interface running normally, vision explain interface address is: {vision_explain}"
                )
            else:
                message = "MCP Vision interface not running normally, please open .config.yaml in data directory, find [server.vision_explain] and set the address"
