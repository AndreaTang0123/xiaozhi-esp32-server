...
class ASRProvider(ASRProviderBase):
    ...
    async def speech_to_text(
        ...
        try:
            ...
            if not self.secret_id or not self.secret_key:
                logger.bind(tag=TAG).error("Tencent Cloud Speech Recognition configuration not set, cannot perform recognition")
    ...
    def _build_request_body(self, base64_audio: str) -> str:
        ...
        request_map = {
            "ProjectId": 0,
            "SubServiceType": 2,  # Sentence recognition
            "EngSerViceType": "16k_zh",  # Mandarin Chinese general
            "SourceType": 1,  # Audio data source is voice file
    ...
    def _get_auth_headers(self, request_body: str) -> Tuple[str, str]:
        ...
        try:
            ...
            # Note: Header information needs to be arranged in ASCII ascending order, and keys and values are converted to lowercase
            # Must include content-type and host headers
            content_type = "application/json; charset=utf-8"
            host = "asr.tencentcloudapi.com"
            action = "SentenceRecognition"  # Interface name

            # Build canonical header information, pay attention to order and format
            canonical_headers = (
